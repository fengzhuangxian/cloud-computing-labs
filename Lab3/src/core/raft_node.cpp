...
namespace raft
{
          ...
          running_(false)
    {
        ...
        if (log_dir_.empty())
        {
            snapshot_file_ = "node_" + std::to_string(node_id_) + "_snapshot.dat";
        }
        else
        {
            snapshot_file_ = log_dir_ + "/node_" + std::to_string(node_id_) + "_snapshot.dat";
        }
    ...
    void RaftNode::Stop()
    {
        ...
        try
        {
            ...
            std::cout << "Saving final snapshot before shutdown..." << std::endl;
            kv_store_->save_snapshot(snapshot_file_);
            std::cout << "Final snapshot saved to: " << snapshot_file_ << std::endl;
        }
        catch (const std::exception &e)
        {
            std::cerr << "Failed to save final snapshot: " << e.what() << std::endl;
        }
    ...
    bool RaftNode::initComponents()
    {
        try
        {
            ...
            // 新增：启动时恢复快照数据
            bool snapshot_restored = kv_store_->restore_from_snapshot(snapshot_file_);
            if (snapshot_restored)
            {
                std::cout << "Successfully restored state from snapshot: "
                          << snapshot_file_ << std::endl;
            }
            else
            {
                std::cout << "No snapshot found or empty snapshot. Starting with empty state."
                          << std::endl;
            }
    ...
    void RaftNode::logApplierLoop()
    {
        ...
        // 添加快照相关变量
        const int SNAPSHOT_THRESHOLD = 1000;                    // 每应用1000条日志保存一次快照
        int last_snapshot_index = raft_core_->getLastApplied(); // 记录上次快照时的索引
        ...
        while (running_)
        {
            ...
            // 新增：定期快照检查（每应用SNAPSHOT_THRESHOLD条日志）
            if (last_applied - last_snapshot_index >= SNAPSHOT_THRESHOLD)
            {
                std::lock_guard<std::mutex> lock(apply_mutex_);
                try
                {
                    std::cout << "Saving snapshot at index: " << last_applied
                              << " to " << snapshot_file_ << std::endl;
                ...
                {
                    std::cerr << "Snapshot save failed: " << e.what() << std::endl;
                }
            }
